...
option_list = list(
  ...
  make_option("--include_all_gene_types", action="store_true", default=FALSE,
              help = "otherwise, only protein_coding lncRNA, and miRNA will be included"),
...
model_training<-function(main_dir,
  ...
  exp_all<-read.table(paste0(expression_file_name),header = T,stringsAsFactors = F, check.names = F)  #keeps IID in genotype file match with IID in expression data
  #gene_list
...
if(opt$asso_test){
  ...
  cov_all = as.data.frame(fread(cov_path))
  
  cat('INFO loading GWAS results ...\n')
  gwas <- as.data.frame(fread(gwas_path))
