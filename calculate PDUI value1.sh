...
function usage(){
	...
	echo "        -t  integer,specify the number of threads used to parallelly running Dapars2,default=24"
...
function generate_configure_for_dapars2(){
	N_cov=$1
	N_threads=$2
	python ../src/02generate_configure_for_Dapars2.py --annotation_3utr ${currDir}/refseq_3utr_annotation.bed \
...
function generate_wigFileList_with_readDepth(){
	...
	python ../src/03extract_read_depth.py --sample_list $bamList --path_wig ${currDir}/wig --output ${currDir}/wigFile_and_readDepth.txt &
	wait
...
function generate_3utr_reference(){
	gene_anno=$1
	refID2Symbol=$2
	python ../src/04DaPars_Extract_Anno.py -b ${gene_anno} -s $refID2Symbol -o ${currDir}/refseq_3utr_annotation.bed &
	wait
